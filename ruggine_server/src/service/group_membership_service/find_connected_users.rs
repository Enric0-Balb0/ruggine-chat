...
impl GroupMembershipService {
    ...
    pub async fn find_connected_users_internal(&self, auth_user_id: i32) -> Result<Vec<i32>, ApiError> {
        let connected_user_ids = self.group_membership_repo.find_connected_users(auth_user_id).await.map_err(|e| {
...
mod find_connected_users_service_tests {
    use crate::error::api_error::ApiError;
    ...
    #[tokio::test]
    async fn test_find_connected_users_database_error() {
        ...
        let result = service.find_connected_users(auth_user_id).await;
