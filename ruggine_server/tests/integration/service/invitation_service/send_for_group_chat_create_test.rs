...
mod invitation_service_send_for_group_chat_create_integration_tests {
    ...
    async fn test_send_for_group_chat_create_success() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target").await;
    ...
    async fn test_send_for_group_chat_create_group_not_found() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target_no_group").await;
    ...
    async fn test_send_for_group_chat_create_user_not_authorized() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target_auth").await;
    ...
    async fn test_send_for_group_chat_create_user_already_in_group() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target_existing").await;
        
        // Create group with admin as creator (this will create membership for admin)
        let group_chat = crate::common::create_test_group_chat_with_invitation_and_membership("invite_gc_create_group_existing", admin_user.id).await;

        // Add target user to the group directly by creating invitation and membership
        let existing_invitation = crate::common::create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
        let _existing_membership = crate::common::create_test_group_membership(existing_invitation.id, target_user.id).await;
        ...
        crate::common::clean_up_group_invitation_with_membership_by_user_id_and_group_chat_id(target_user.id, group_chat.id).await;
        cleanup_group_chat(group_chat.id).await;
        cleanup_user(admin_user.email).await;
        cleanup_user(target_user.email).await;
    }
    ...
    async fn test_send_for_group_chat_create_already_invitation_pending() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target_pending").await;
    ...
    async fn test_send_for_group_chat_create_with_admin_role() {
        ...
        let (target_user, _) = create_test_user("invite_gc_create_target_role_admin").await;
