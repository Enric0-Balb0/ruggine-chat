use ruggine_server::handler::invitation_handler::find_by_id::find_by_id;
...
mod find_by_id_handler_integration_tests {
    ...
    async fn test_find_by_id_handler_returns_invitation_successfully() {
        // Arrange: Create users, group chat and invitation
        let (admin_user, _password) = create_test_user("find_by_id_success_admin").await;
        let group_chat = create_test_group_chat("find_by_id_success_group", admin_user.id).await;
        let (target_user, _password) = create_test_user("find_by_id_success_target").await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_handler_fails_when_invitation_not_found() {
        // Arrange: Create user and invitation state
        let (current_user, _password) = create_test_user("find_by_id_not_found_user").await;
        let invitation_state = create_invitation_state().await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_handler_returns_different_invitations() {
        // Arrange: Create users, group chats and invitations
        let (admin_user1, _password) = create_test_user("find_by_id_different_admin1").await;
        let group_chat1 = create_test_group_chat("find_by_id_different_group1", admin_user1.id).await;
        let (target_user1, _password) = create_test_user("find_by_id_different_target1").await;
        let invitation1 = create_test_invitation(admin_user1.id, target_user1.id, group_chat1.id).await;
        
        let (admin_user2, _password) = create_test_user("find_by_id_different_admin2").await;
        let group_chat2 = create_test_group_chat("find_by_id_different_group2", admin_user2.id).await;
        let (target_user2, _password) = create_test_user("find_by_id_different_target2").await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_handler_dont_works_with_any_authenticated_user() {
        // Arrange: Create users, group chat and invitation
        let (admin_user, _password) = create_test_user("find_by_id_any_user_admin").await;
        let group_chat = create_test_group_chat("find_by_id_any_user_group", admin_user.id).await;
        let (target_user, _password) = create_test_user("find_by_id_any_user_target").await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
        
        let (random_user, _password) = create_test_user("find_by_id_any_user_random").await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_handler_returns_invitation_with_specific_data() {
        // Arrange: Create users, group chat and invitation with specific data
        let (admin_user, _password) = create_test_user("find_by_id_specific_admin").await;
        let group_chat = create_test_group_chat("find_by_id_specific_group", admin_user.id).await;
        let (target_user, _password) = create_test_user("find_by_id_specific_target").await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
