...
use crate::common::{get_database, create_test_user, cleanup_user_by_email, cleanup_group_chat, cleanup_invitation, create_test_invitation, cleanup_group_membership};

#[cfg(test)]
mod group_membership_repository_promote_admin_if_none_integration_tests {
    use chrono::Utc;
    ...
    async fn test_promote_admin_if_none_success_single_member() {
        ...
        let (owner_user, _) = create_test_user("promote_admin_owner").await;
        let (member_user, _) = create_test_user("promote_admin_member").await;
        
        // Create group and membership for regular member
        let group_chat = create_test_group_chat("promote_admin_group", owner_user.id).await;
        ...
        let result = repository.promote_admin_if_none(group_chat.id).await;
    ...
    async fn test_promote_admin_if_none_no_promotion_when_admin_exists() {
        ...
        // Create users and group
        let (owner_user, _) = create_test_user("no_promote_owner").await;
        let (admin_user, _) = create_test_user("no_promote_admin").await;
        let (member_user, _) = create_test_user("no_promote_member").await;
        
        // Create group and memberships
        let group_chat = create_test_group_chat("no_promote_group", owner_user.id).await;
    ...
    async fn test_promote_admin_if_none_multiple_members_promotes_first() {
        ...
        // Create users and group
        let (owner_user, _) = create_test_user("multi_promote_owner").await;
        let (member1_user, _) = create_test_user("multi_promote_member1").await;
        let (member2_user, _) = create_test_user("multi_promote_member2").await;
        let (member3_user, _) = create_test_user("multi_promote_member3").await;
        
        // Create group and invitations
        let group_chat = create_test_group_chat("multi_promote_group", owner_user.id).await;
    ...
    async fn test_promote_admin_if_none_no_active_members() {
        ...
        // Create users and group
        let (owner_user, _) = create_test_user("no_active_owner").await;
        let group_chat = create_test_group_chat("no_active_group", owner_user.id).await;
    ...
    async fn test_promote_admin_if_none_only_inactive_members() {
        ...
        // Create users and group
        let (owner_user, _) = create_test_user("inactive_owner").await;
        let (member_user, _) = create_test_user("inactive_member").await;
        
        // Create group and membership
        let group_chat = create_test_group_chat("inactive_group", owner_user.id).await;
        let invitation = create_test_invitation(owner_user.id, member_user.id, group_chat.id).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_promote_admin_if_none_nonexistent_group() {
        ...
        let result = repository.promote_admin_if_none(nonexistent_group_id).await;

        // Assert: Should succeed but not promote anyone
        assert!(result.is_ok(), "Function should succeed even for non-existent group");
        
        let promoted_membership = result.unwrap();
        assert!(promoted_membership.is_none(), "Should not promote anyone for non-existent group");
    }
    ...
    async fn test_promote_admin_if_none_ignores_left_members() {
        ...
        // Create users and group
        let (owner_user, _) = create_test_user("left_owner").await;
        let (active_user, _) = create_test_user("left_active").await;
        let (left_user, _) = create_test_user("left_left").await;
        
        // Create group and invitations
        let group_chat = create_test_group_chat("left_group", owner_user.id).await;
