...
async fn test_find_info_by_id_success() {
    ...
    // Act
    let result = repository.find_info_by_id(info_id).await;
...
async fn test_find_info_by_id_not_found() {
    ...
    // Act
    let result = repository.find_info_by_id(non_existent_id).await;
...
async fn test_find_info_by_id_multiple_infos_same_message() {
    ...
    let result1 = repository.find_info_by_id(info1_id).await;
    let result2 = repository.find_info_by_id(info2_id).await;
...
#[tokio_shared_rt::test(shared)]
async fn test_find_info_by_id_with_negative_id() {
    // Arrange
    let db = common::get_database().await;
    let repository = TextMessageRepository::new(&db);
    let negative_id = -1;
    ...
    // Act
    let result = repository.find_info_by_id(negative_id).await;
...
#[tokio_shared_rt::test(shared)]
async fn test_find_info_by_id_zero_id() {
    // Arrange
    let db = common::get_database().await;
    let repository = TextMessageRepository::new(&db);
    let zero_id = 0;
    ...
    // Act
    let result = repository.find_info_by_id(zero_id).await;
