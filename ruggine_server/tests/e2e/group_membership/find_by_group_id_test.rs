...
#[cfg(test)]
mod find_by_group_id_e2e_tests {
    use crate::{
    ...
    async fn test_find_by_group_id_e2e_success_multiple_members() {
        // Arrange: Create admin with group and add multiple members
        let (admin, _admin_pw, admin_token) = create_login_and_get_token("find_by_group_id_multiple_admin".to_string()).await;
        let group = create_test_group_chat_with_invitation_and_membership("find_by_group_id_multiple_group", admin.id).await;
        let members = create_test_users_for_a_group("find_by_group_id_multiple_member", 3, &group).await;
        ...
        let request = Request::builder()
            .method("GET")
            .uri(&format!("/api/group_membership/group/{}", group.id))
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_group_id_e2e_success_single_member() {
        // Arrange: Create admin with group and add single member
        let (admin, _admin_pw, admin_token) = create_login_and_get_token("find_by_group_id_single_admin".to_string()).await;
        let group = create_test_group_chat_with_invitation_and_membership("find_by_group_id_single_group", admin.id).await;
        let members = create_test_users_for_a_group("find_by_group_id_single_member", 1, &group).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_group_id_e2e_unauthorized() {
        ...
        let request = Request::builder()
            .method("GET")
            .uri("/api/group_membership/group/123")
            .body(Body::empty())
            .unwrap();
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_group_id_e2e_forbidden_not_member() {
        // Arrange: Create non-member user and admin with group
        let (non_member, _non_member_pw, non_member_token) = create_login_and_get_token("find_by_group_id_non_member".to_string()).await;
        let (admin, _admin_pw, _admin_token) = create_login_and_get_token("find_by_group_id_admin_forbidden".to_string()).await;
        let group = create_test_group_chat_with_invitation_and_membership("find_by_group_id_forbidden_group", admin.id).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_group_id_e2e_group_not_found() {
        // Arrange: Create user and login
        let (user, _user_pw, token) = create_login_and_get_token("find_by_group_id_no_group".to_string()).await;
        let app = create_full_router().await;
        ...
        let request = Request::builder()
            .method("GET")
            .uri(&format!("/api/group_membership/group/{}", non_existent_group_id))
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_group_id_e2e_after_leaving_group() {
        // Arrange: Create admin with group and add member
        let (admin, _admin_pw, _admin_token) = create_login_and_get_token("find_by_group_id_leave_admin".to_string()).await;
        let group = create_test_group_chat_with_invitation_and_membership("find_by_group_id_leave_group", admin.id).await;
        let (member, member_pw, member_token) = create_login_and_get_token("find_by_group_id_leave_member".to_string()).await;
