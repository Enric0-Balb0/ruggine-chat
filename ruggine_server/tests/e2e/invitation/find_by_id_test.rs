...
#[cfg(test)]
mod find_by_id_invitation_e2e_tests {
    use super::*;

    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_success_as_from_user() {
        // Arrange: Create router, users, group chat, and invitation
        let app = create_invitation_router().await;
        let (admin_user, _password, token) = create_login_and_get_token("e2e_find_success_admin".to_string()).await;
        let group_chat = create_test_group_chat("e2e_find_success_group", admin_user.id).await;
        let (target_user, _target_password) = create_test_user("e2e_find_success_target").await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_success_as_to_user() {
        // Arrange: Create router, users, group chat, and invitation
        let app = create_invitation_router().await;
        let (admin_user, _password, _token) = create_login_and_get_token("e2e_find_to_user_admin".to_string()).await;
        let group_chat = create_test_group_chat("e2e_find_to_user_group", admin_user.id).await;
        let (target_user, _target_password, target_token) = create_login_and_get_token("e2e_find_to_user_target".to_string()).await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_unauthorized_no_token() {
        // Arrange: Create router, users, group chat, and invitation
        let app = create_invitation_router().await;
        let (admin_user, _password, _token) = create_login_and_get_token("e2e_find_no_auth_admin".to_string()).await;
        let group_chat = create_test_group_chat("e2e_find_no_auth_group", admin_user.id).await;
        let (target_user, _target_password) = create_test_user("e2e_find_no_auth_target").await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_unauthorized_invalid_token() {
        // Arrange: Create router, users, group chat, and invitation
        let app = create_invitation_router().await;
        let (admin_user, _password, _token) = create_login_and_get_token("e2e_find_invalid_auth_admin".to_string()).await;
        let group_chat = create_test_group_chat("e2e_find_invalid_auth_group", admin_user.id).await;
        let (target_user, _target_password) = create_test_user("e2e_find_invalid_auth_target").await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_not_found_invalid_id() {
        // Arrange: Create router and user
        let app = create_invitation_router().await;
        let (user, _password, token) = create_login_and_get_token("e2e_find_not_found_user".to_string()).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_not_found_unauthorized_user() {
        // Arrange: Create router, users, group chat, and invitation
        let app = create_invitation_router().await;
        let (admin_user, _password, _token) = create_login_and_get_token("e2e_find_unauthorized_admin".to_string()).await;
        let group_chat = create_test_group_chat("e2e_find_unauthorized_group", admin_user.id).await;
        let (target_user, _target_password) = create_test_user("e2e_find_unauthorized_target").await;
        let invitation = create_test_invitation(admin_user.id, target_user.id, group_chat.id).await;
        
        // Create a third user who is not involved in the invitation
        let (random_user, _random_password, random_token) = create_login_and_get_token("e2e_find_unauthorized_random".to_string()).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_bad_request_invalid_id_format() {
        // Arrange: Create router and user
        let app = create_invitation_router().await;
        let (user, _password, token) = create_login_and_get_token("e2e_find_invalid_format_user".to_string()).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_bad_request_negative_id() {
        // Arrange: Create router and user
        let app = create_invitation_router().await;
        let (user, _password, token) = create_login_and_get_token("e2e_find_negative_id_user".to_string()).await;
    ...
    #[tokio_shared_rt::test(shared)]
    async fn test_find_by_id_invitation_returns_different_invitations() {
        // Arrange: Create router, users, group chats, and invitations
        let app = create_invitation_router().await;
        let (admin_user1, _password1, token1) = create_login_and_get_token("e2e_find_different_admin1".to_string()).await;
        let group_chat1 = create_test_group_chat("e2e_find_different_group1", admin_user1.id).await;
        let (target_user1, _target_password1) = create_test_user("e2e_find_different_target1").await;
        let invitation1 = create_test_invitation(admin_user1.id, target_user1.id, group_chat1.id).await;
        
        let (admin_user2, _password2, token2) = create_login_and_get_token("e2e_find_different_admin2".to_string()).await;
        let group_chat2 = create_test_group_chat("e2e_find_different_group2", admin_user2.id).await;
        let (target_user2, _target_password2) = create_test_user("e2e_find_different_target2").await;
