// Integration test: ensure the invitations_list_modal empty state text is the expected one
#[cfg(test)]
mod invite_empty_message_test {
    ...
    fn empty_invites_shows_received_text() {
        ...
        let path = "src/components/modals/invitations_list_modal.rs";
        let content = fs::read_to_string(path).expect("failed to read invitations_list_modal.rs");
